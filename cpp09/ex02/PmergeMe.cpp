...
VectSort::VectSort(const VectSort &other) : _data(other._data), _start(other._start), _order(other._order){}
...
void DequeSort::endSortInfo() const{
	...
	clock_t end = clock() - _start;
	std::cout << "After:\t";
	for (de::const_iterator it = _data.begin(), end = _data.end(); it != end;){
		std::cout << *it;
		it++;
		if (it != end)
			std::cout << " ";
...
DequeSort::DequeSort(const DequeSort &other) : _data(other._data), _start(other._start), _order(other._order){}
...
void DequeSort::debugInsert(de &main, de &pend) const{
...
std::vector<long> DequeSort::jacobsthal(size_t size) const{
	std::vector<long> j;
	size_t range = 0;
...
void DequeSort::debugBinary(de &main, det sbegin, det send, det begin, det end, det mid, int range, int mode){
	...
	if (mode == 1){
		...
		std::cout << "main:\t";
		size_t j = 0;
		for (det i = main.begin(); i != main.end(); i++){
			if (j == 0)
				std::cout << "{";
			j++;
			if (i == begin)
...
void DequeSort::binarySearch(de &main, det sbegin, det send, det begin, det end){
...
void DequeSort::standardBinarySearch(de &main, de &pend){
	det sbegin = pend.end() - _order;
	det send = pend.end() - 1;
...
void DequeSort::jacobsthalBinarySearch(de &main, de &pend, std::vector<long>::iterator &j){
	...
	while (!pend.empty()){
		...
		while (nbrInsert > 0){
			...
			int jRange = (*j + (*(j - 1))) - 1;
			det sbegin = pend.begin() + (_order * nbrInsert) - _order;
			det send = pend.begin() + (_order * nbrInsert) - 1;
			det begin = main.begin();
			int range = (main.size() / _order) - jRange;
			det end = main.end() - (range * _order) - 1;
			if (DEBUG && (!_debug || _debug == 2)){
				std::cout << "will be inserted:\t";
				for (det i = sbegin; i != send + 1; i++){
					std::cout << *i;
					if (i != send)
						std::cout << ", ";
				}
...
void DequeSort::sort(de &main, de &pend, de &trash){
	std::vector<long> j = jacobsthal(pend.size() / _order);
	if (DEBUG && (!_debug || _debug == 2)){
		std::cout << "\nJ: ";
		for (size_t i = 0; i < j.size(); i++){
	...
	if (it == j.end())
		...
		jacobsthalBinarySearch(main, pend, it);
	if (!trash.empty()){
		for (det ite = trash.begin(); ite != trash.end(); ite++)
			main.push_back(*ite);
		trash.clear();
	}
...
void DequeSort::FordJonhson(){
	...
	std::cout << "Before:\t";
	for (det it = _data.begin(), end = _data.end(); it != end;){
		std::cout << *it;
		it++;
		if (it != end)
			std::cout << " ";
